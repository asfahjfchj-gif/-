...
local Locale = {
    ...
    -- ПЕРФГАРД / СОРТИРОВКА / БИНДЫ / ТЕМЫ
...
-- FOV Circle
local function SendNotify(msg) pcall(function() game:GetService("StarterGui"):SetCore("SendNotification", {Title="Ultimate Hack V8", Text=msg, Duration=3}) end) end
...
-- Auto-refresh player list every 60 seconds
task.spawn(function()
...
local function GetWorldDescendants(now)
    if (not worldDescCache) or ((now - worldDescCacheAt) >= 0.5) then
...
RunService.RenderStepped:Connect(function()
    ...
    -- No Recoil
    if Settings.Combat.NoRecoil and ShouldRunFeature("NoRecoil", 0.15, now) then
        local char = LocalPlayer.Character
        if char then
    ...
    -- LOOP TP (supports multiple selected targets)
    if Settings.TP.LoopTP then
    ...
    -- Persistent Spectate (survives target death/respawn) - camera only, no character teleport
    ...
    if char and char:FindFirstChild("HumanoidRootPart") then
        ...
        -- Follow Player (Movement tab - legacy)
        ...
        -- Orbit Player (Movement tab - legacy)
        ...
        -- World Freeze
        if Settings.World.WorldFreeze and ShouldRunFeature("WorldFreeze", 0.5, now) then
            for _,v in ipairs(GetWorldDescendants(now)) do
        ...
        if Settings.World.RemoveParticles and ShouldRunFeature("RemoveParticles", 0.35, now) then
            for _,v in ipairs(GetWorldDescendants(now)) do
        ...
        -- Big Head
        if Settings.World.BigHead and ShouldRunFeature("BigHead", 0.2, now) then
            for _,p in ipairs(Players:GetPlayers()) do
        ...
        -- Small Players
        if Settings.World.SmallPlayers and ShouldRunFeature("SmallPlayers", 0.6, now) then
            for _,p in ipairs(Players:GetPlayers()) do
        ...
        -- Bright Players
        if Settings.ESP.BrightPlayers and ShouldRunFeature("BrightPlayers", 0.35, now) then
            for _,p in ipairs(Players:GetPlayers()) do
                if p ~= LocalPlayer and p.Character then
        ...
        if Settings.Combat.HitboxExtenderArms and ShouldRunFeature("HitboxExtenderArms", 0.3, now) then for _,p in ipairs(Players:GetPlayers()) do if p ~= LocalPlayer and p.Character then for _,v in ipairs(p.Character:GetChildren()) do if v:IsA("BasePart") and (string.find(v.Name, "Arm") or string.find(v.Name, "Leg")) then v.Size = Vector3.new(3,3,3) end end end end end
        
        -- ESP/Visuals V2
        if Settings.ESP.ChamsMaterial ~= "Neon" and Settings.ESP.Chams and ShouldRunFeature("ChamsMaterial", 0.4, now) then for _,p in ipairs(Players:GetPlayers()) do if p ~= LocalPlayer and p.Character then for _,v in ipairs(p.Character:GetDescendants()) do if v:IsA("BasePart") then pcall(function() v.Material = Enum.Material[Settings.ESP.ChamsMaterial] end) end end end end end
        ...
        if Settings.ESP.WallHackV2 and ShouldRunFeature("WallHackV2", 0.5, now) then for _,v in ipairs(GetWorldDescendants(now)) do if v:IsA("BasePart") and v.Transparency == 0 and not Players:GetPlayerFromCharacter(v.Parent) then v.Transparency = 0.5 end end end
        
        -- World V2
        if Settings.World.ClearWeather then pcall(function() Workspace.Terrain.Clouds.Enabled = false end); Lighting.FogEnd = 99999 end
        if Settings.World.NoTextures and ShouldRunFeature("NoTextures", 0.7, now) then for _,v in ipairs(GetWorldDescendants(now)) do if v:IsA("BasePart") and not Players:GetPlayerFromCharacter(v.Parent) then v.Material = Enum.Material.SmoothPlastic end end end
        ...
        if Settings.World.BouncyWorld and ShouldRunFeature("BouncyWorld", 0.8, now) then for _,v in ipairs(GetWorldDescendants(now)) do if v:IsA("BasePart") and not Players:GetPlayerFromCharacter(v.Parent) then v.CustomPhysicalProperties = PhysicalProperties.new(0.5, 0.3, 1, 1, 1) end end end
        if Settings.World.SlipperyWorld and ShouldRunFeature("SlipperyWorld", 0.8, now) then for _,v in ipairs(GetWorldDescendants(now)) do if v:IsA("BasePart") and not Players:GetPlayerFromCharacter(v.Parent) then v.CustomPhysicalProperties = PhysicalProperties.new(0.5, 0, 0, 1, 1) end end end
...
-- ============================================================
-- // КОЛЕСО ЭМОЦИЙ (загрузка из отдельного файла)
-- ============================================================
task.spawn(function()
    local ok, err = pcall(function()
