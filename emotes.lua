-- Emote Wheel (standalone) — 996 emotes
...
-- Стиль (codex.lua / Ultimate Hack V8)
local S = {
...
-- Глобальные для codex.lua кнопок
getgenv().__StopEmote = StopCur
getgenv().__emoteCurrentTrack = nil -- обновляется через метатаблицу ниже
